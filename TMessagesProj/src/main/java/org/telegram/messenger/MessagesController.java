...
import org.telegram.myUtil.FavoriteUtil;
import org.telegram.myUtil.UpdateAvatar.UpdateBiz;
...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public ArrayList<TLRPC.Dialog> dialogsMegaGroups = new ArrayList<>();
        ...
        if (first) {
            ...
            if (dialog != null) {
                ...
                if (onlyHistory == 0) {
                    ...
                    dialogsJustGroupsOnly.remove(dialog);
                    dialogsBotOnly.remove(dialog);
                    dialogsContactOnly.remove(dialog);
                    dialogsChannelOnly.remove(dialog);
    ...
    public void updateTimerProc() {
        ...
        if (UserConfig.isClientActivated()) {
            if (ApplicationLoader.INVISIBLE_MODE) {  // invisible  mode
            ...
            } else if (ConnectionsManager.getInstance().getPauseTime() == 0 && ApplicationLoader.isScreenOn && !ApplicationLoader.mainInterfacePaused) {
                ...
                AndroidUtilities.runOnUIThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        for (int a = 0; a < dialogs.size(); a++) {
                            ...
                            if (FavoriteUtil.isFavorite(d.id)) {
...
//                        if (ApplicationLoader.SHOW_TAB && (ApplicationLoader.VISIBLE_TAB_MASK & MessagesController.UPDATE_MASK_CHAT_MEMBERS) != 0) {
                        if (currentDialog.unread_count == 0) {
                            dialogsUnreadOnly.remove(currentDialog);
                        } else if (!dialogsUnreadOnly.contains(currentDialog)) {
                            dialogsUnreadOnly.add(currentDialog);
                        }
//                        }
                    }
                }
    ...
    public boolean processUpdateArray(ArrayList<TLRPC.Update> updates, final ArrayList<TLRPC.User> usersArr, final ArrayList<TLRPC.Chat> chatsArr) {
            ...
            if (update instanceof TLRPC.TL_updateNewMessage || update instanceof TLRPC.TL_updateNewChannelMessage) {
                ...
                if (checkForUsers) {
                    ...
                    if (chat_id != 0) {
                        ...
                        if (chat.left && ApplicationLoader.DELETE_DIALOG_ON_LEAVE) {
                            continue;
                        }
                    }
                ...
                if (message.action instanceof TLRPC.TL_messageActionChatDeleteUser) {
                    ...
                    if (user != null && user.bot) {
                        message.reply_markup = new TLRPC.TL_replyKeyboardHide();
                    } else if (message.from_id == UserConfig.getClientUserId() && message.action.user_id == UserConfig.getClientUserId()&& ApplicationLoader.DELETE_DIALOG_ON_LEAVE) {
                        continue;
                    }
                } else if (message.action instanceof TLRPC.TL_messageActionChatMigrateTo || message.action instanceof TLRPC.TL_messageActionChannelCreate) {
                    message.unread = false;
        ...
        ApplicationLoader.DELETE_DIALOG_ON_LEAVE = true;
        if (!messages.isEmpty()) {
            for (HashMap.Entry<Long, ArrayList<MessageObject>> pair : messages.entrySet()) {
                Long key = pair.getKey();
                ArrayList<MessageObject> value = pair.getValue();
